...
# global constants
GAME_ASPECTS = ['Gameplay', 'Narrative', 'Accessibility', 'Sound',
                'Graphics & Art Design', 'Performance', 'Bug', 'Suggestion', 'Price', 'Overall']
...
chroma_client = chromadb.PersistentClient(
    path=str(llm_rag_folder.joinpath('chromadb_storage')))
...
def _check_spam(review: str) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        # default return true (i.e. isSpam)
    ...
    chat_prompt_02 = ChatPromptTemplate.from_messages([
        ("system", _prompts.SYSTEM_TEMPLATE),
        ("human", _prompts.SPAM_TEMPLATE_01),
        # .content to get the response, same as below
        ("ai", response_01.content),
...
def _get_aspect_content_per_review(review: str) -> Tuple[Dict, Dict]:
    ...
    db, retriever, embedding_func, embedding_usage_info_01 = _get_rag_documents(
        _rag_request)
    ...
    aspects_response, chain_llm_output_json_list, embedding_usage_info_list = _get_aspects_content(
        db, retriever, embedding_func)
...
def _get_rag_documents(_rag_request: dict):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        llm_rag_folder.joinpath("Mixtral-8x7B-Instruct-v0.1_tokenizer"))
    text_splitter = RecursiveCharacterTextSplitter.from_huggingface_tokenizer(
        tokenizer, chunk_size=250, chunk_overlap=40)
    ...
    if _rag_request['type'] == RagType.PER_REVIEW:
        ...
        docs = text_splitter.create_documents(
            [review], metadatas=[{"source": "review_01"}])
        ...
        try:
            # get the collection from the docker client (the docker client provides persistent memory)
            db = Chroma(collection_name=_collection_name,
                        client=chroma_client, embedding_function=embedding_func)
        except:
            print(
                f'Failed to create a db for the collection {_collection_name}. Exiting...')
            print(traceback.format_exc())
            db = None
...
def _get_aspects_content(db, retriever, embedding_func) -> Tuple[Dict, List, List]:
    ...
    n_docs = len(db.get()['documents'])
    _print_message(
        f'Number of documents in the collection @_get_aspects_content: {n_docs}')

    # if the text exceeds roughtly 1000 words, then we get the aspects one by one
    # else, we get all aspects at once
    if n_docs <= 10:
        aspects_response, chain_llm_output_json_list, embedding_usage_info_list = _get_aspects_334(
            retriever, embedding_func)
    else:
        aspects_response, chain_llm_output_json_list, embedding_usage_info_list = _get_aspects_10(
            retriever, embedding_func)
...
def _get_aspects_334(retriever, embedding_func) -> Tuple[Dict, List, List]:
    ...
    for (start, end) in [(0, 3), (3, 6), (6, 10)]:
        ...
        my_question = _prompts.QUESTION_TEMPLATE_01 + \
            f"{'is ' if len(aspects) <= 1 else 'are '}" + ': ' + f'{aspects}'
        ...
        relevant_docs = retriever.get_relevant_documents(
            query=my_question, k=5)
        embedding_stats = embedding_func.get_embedding_stats_queue(
        ).popleft()           # get embedding token usage
        _print_message(f'Embedding stats: {embedding_stats}')
        embedding_usage_info_01 = _process_embedding_stats(embedding_stats)
        ...
        for i in range(5):
            ...
            token_usage_json_list.append(
                _resp.response_metadata['token_usage'])
            ...
            if (open_brace_count <= 0 and close_brace_count <= 0) or (open_brace_count < 1):
                _print_message(
                    f'open_brace_count: {open_brace_count} or close_brace_count: {close_brace_count}. Retry...')
                continue
            ...
            # '''Gameplay: The game is challenging with a variety of weapons and builds to try. Bosses can be frustrating but are ultimately fun and fair. Platforming sections are less successful, but rare. ({"Gameplay": "The game is challenging with a variety of weapons and builds to try. Bosses can be frustrating but are ultimately fun and fair. Platforming sections are less successful, but rare."})\n\nNarrative: The game features a fantastic narrative that becomes more rewarding as you progress, with new secrets to discover during each playthrough. ({"Narrative": "The game features a fantastic narrative that becomes more rewarding as you progress, with new secrets to discover during each playthrough."})\n\nAccessibility: The game is not very accessible for players new to the genre, with a steep learning curve and some frustrating elements. ({"Accessibility": "The game is not very accessible for players new to the genre, with a steep learning curve and some frustrating elements."})''''
                ...
                if open_brace_count != close_brace_count:
                    _print_message(
                        f'open_brace_count: {open_brace_count} != close_brace_count: {close_brace_count}. Retry...')
                    continue

                # manually get the JSON object by finding each aspect in the response
                # have to consider both single and double quotes
                # as mistral AI sometimes uses single quotes and sometimes double quotes
                _parsing_json_multiple(
                    resp, open_brace_count, aspects_response, aspects)
...
def _get_sentiment_per_aspect_per_review(review: str, is_spam: bool, aspects_response: dict) -> Tuple[Dict, Dict]:
    ...
    for (start, end) in ((0, 3), (3, 6), (6, 10)):
        ...
        output_format = "Output the JSON as a single line with no spaces between the key, value pairs. For example, if the aspects are {}, the JSON should be: {}".format(
            str(aspects)[1:-1].replace('\'',
                                       '\"'), str(output_json_template).replace('\'', '\"')
        )
        ...
        for i in range(repeat_limit):
            ...
            _print_message(
                f'LLM result in _get_sentiment_per_aspect_per_review: {_resp}')
            token_usage_json_list.append(
                _resp.response_metadata['token_usage'])
            ...
            # '''Gameplay: The game is challenging with a variety of weapons and builds to try. Bosses can be frustrating but are ultimately fun and fair. Platforming sections are less successful, but rare. ({"Gameplay": "The game is challenging with a variety of weapons and builds to try. Bosses can be frustrating but are ultimately fun and fair. Platforming sections are less successful, but rare."})\n\nNarrative: The game features a fantastic narrative that becomes more rewarding as you progress, with new secrets to discover during each playthrough. ({"Narrative": "The game features a fantastic narrative that becomes more rewarding as you progress, with new secrets to discover during each playthrough."})\n\nAccessibility: The game is not very accessible for players new to the genre, with a steep learning curve and some frustrating elements. ({"Accessibility": "The game is not very accessible for players new to the genre, with a steep learning curve and some frustrating elements."})''''
                ...
                # as mistral AI sometimes uses single quotes and sometimes double quotes
                _parsing_json_multiple(
                    resp, open_brace_count, aspects_resp_sentiment, aspects)
...
def _gen_keywords_per_review(review: str, is_spam: bool, aspects_response: dict) -> Tuple[Dict, Dict]:
    ...
    for i in range(repeat_limit):
        ...
        try:
            ...
            print(
                f'response_02: \'\'\'{response_02}\'\'\' is not a JSON. Resort to manual parse...')
        ...
        for i, aspect in enumerate(GAME_ASPECTS):
            ...
            if i != len(GAME_ASPECTS) - 1:
                ...
                next_aspect_start = max(response_02.find(
                    f'\"{next_aspect}\"'), response_02.find(f'\'{next_aspect}\''))
                if next_aspect_start == -1:
                    print(
                        f'next_aspect: {next_aspect} not in resp_ans. Retry...')
                    continue
            else:
                next_aspect_start = len(response_02)

            resp_start = max(response_02.find(f'\"{aspect}\"'), response_02.find(
                f'\'{aspect}\'')) + len(f'\"{aspect}\"')
            ...
            quotes_a = [i for i, x in enumerate(
                response_02) if x == '\"' and value_start < i < value_end]
            quotes_b = [i for i, x in enumerate(
                response_02) if x == '\'' and value_start < i < value_end]
            quotes = quotes_a if quotes_a else quotes_b

            # then extract the words btw the quotes
            keywords = [response_02[quotes[i]+1:quotes[i+1]]
...
def get_per_review_analysis(review: str) -> Tuple[bool, Dict, Dict, str, Dict]:
    ...
    if not is_spam:
        aspects_response, token_usage_stats = _get_aspect_content_per_review(
            review)
    else:
    ...
    aspect_keywords, token_usage_stats = _gen_keywords_per_review(
        review, is_spam, aspects_response)
    token_usage_stats_list.append(token_usage_stats)

    # get sentiment for each aspect
    aspect_sentiment, token_usage_stats = _get_sentiment_per_aspect_per_review(
        review, is_spam, aspects_response)
    token_usage_stats_list.append(token_usage_stats)
    ...
    tldr, token_usage_stats = _gen_TLDR_per_review(
        review, is_spam, aspects_response)
    token_usage_stats_list.append(token_usage_stats)
...
def gen_TLDR_per_game(game_id: int) -> Tuple[str, Dict]:
    ...
    gameAnalystic_json = _pergame_tldr._get_gameAnalystic_result_from_api(
        game_id)
    ...
    aspect_content_per_game, token_usage_stats = _get_aspect_content_per_game(
        game_name)
    token_usage_stats_list.append(token_usage_stats)
    ...
    # step 5: the prompt
    tldr, token_usage_stats = _gen_TLDR_per_game(
        aspect_content_per_game, sa_stats, topN_topicnames)
    token_usage_stats_list.append(token_usage_stats)
...
def _get_sa_stats(dfs: dict, llm_model) -> Tuple[Dict, Dict]:
    ...
    assert set(_pergame_tldr.PROMPT_PER_DFS.keys()) <= set(
        dfs.keys()), f'{set(_pergame_tldr.PROMPT_PER_DFS.keys())} <= {set(dfs.keys())}'
    ...
    for df_key in dfs.keys() & _pergame_tldr.PROMPT_PER_DFS.keys():
        ...
        try:
            ...
            _print_message(
                'LLM result in _get_sa_stats, of {}: {}'.format(df_key, _resp_01))
