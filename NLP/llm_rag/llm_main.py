...
from mistralai_chatmodels import CustomChatMistralAI
...
llm_mistralai = CustomChatMistralAI(
...
def _check_spam(review:str) -> Tuple[bool, Dict]:
    ...
    token_usage_list.append(response_01.response_metadata.get('token_usage', DEFAULT_TOKEN_USAGE))
    ...
    token_usage_list.append(response_02.response_metadata.get('token_usage', DEFAULT_TOKEN_USAGE))
...
def _gen_TLDR_per_review(review:str, is_spam:bool, aspects_response:dict) -> Tuple[str, Dict]:
    ...
    token_usage_list.append(_resp.response_metadata.get('token_usage', DEFAULT_TOKEN_USAGE))
...
def _get_sa_stats(dfs:dict, llm_model) -> Tuple[Dict, Dict]:
    ...
    for df_key in dfs.keys() & _pergame_tldr.PROMPT_PER_DFS.keys():
        ...
        try:
            ...
            token_usage_list.append(_resp_01.response_metadata.get('token_usage', DEFAULT_TOKEN_USAGE))
...
if __name__ == "__main__":
    ...
    game_name = 'Starfield'
    game_id = 8         # the id in our platform

    pergame_tldr, token_usage_breakdown = gen_TLDR_per_game(game_id)
