...
## API (for NLP tm related)

(bertopic)
1. Return the topic id, topic name. (others required?? like top1 or top5 topic id and their prob)  
Input: review, game name, game genre (list)

    a. Select game genre: action, indie -> load bertopic model (for action/indie)
        (and)

    b. Check if game is a game with many comments -> bertopic[split] ntopics = 10
    else: bertopic[split] ntopics=30

    c. return topic id, topic name (or anything u want)

2. LLM for individual reviews (including per critic review)  
Input: game name, game description  
Return the JSON (with a list of keywords/phrases for each aspects)  

    a. prompt LLM to determine if the review is a spam given game name and description

    b. If spam then simply return NONE

    c. Else prompt LLM to generate the JSON with a list of keywords/phrases from each aspects.

3. LLM for TLDR (per review)  
Return 50 >= short description generated by LLM

    a. prompt LLM to determine if the review is a spam given game name and description, and also count number of words (200 words). at least 200 words to trigger this

    b. return a 50>= TLDR for the review

4. LLM for TLDR (per game) -> Placed in Aggregated Review  
Input: game name, description, top frequency (select most common topics name), sentiment analysis results (tbc)  
Output: a JSON with a list of short description for each aspects

    a. Go to chromadb to find the collection (subdata) with all "critic reviews" (a separate collection)

    b. Prompt LLM to generate the JSON with a list of short description (or keywords??) from those critic reviews.

    c. Then also ask for sentiment analysis stats (like ratio of reviewers in diff age, gender distribution), and topic modeling topic frequency.

    d. Return a TLDR per game for displaying in the "game page"
